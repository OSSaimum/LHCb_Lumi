...
# set the directory to the location where the ee files are located
# execute if specific ee bunch crossing info is needed
dir.to.ee.files <- 'C:\\Users\\saimum\\R\\plume\\june_ee'

setwd("C:/Users/saimum/R/plume/june_ee")

jeesdt <- data.table(file = list.files(dir.to.ee.files, pattern='^run.*gz')
                  )[, {
                        . <- fread(file, col.names =  c('minute','name','bx','x','y'))[name=='VeloVertices']
                    }, by = .(file)]
...
names = list('ECalET','ECalEtot','ECalETOuterTop', 'ECalETOuterBottom', 'ECalETMiddleTop', 'ECalETMiddleBottom', 'ECalETInnerTop', 'ECalETInnerBottom',
  'RawECalEOuterTop', 'RawECalEOuterBottom', 'RawECalEMiddleTop', 'RawECalEMiddleBottom', 'RawECalEInnerTop', 'RawECalEInnerBottom', "MuonHitsM2R1",   
  'MuonHitsM2R2',  'MuonHitsM2R3',  'MuonHitsM2R4',  'MuonHitsM3R1',  'MuonHitsM3R2',  'MuonHitsM3R3',  'MuonHitsM3R4',  'MuonHitsM4R1',  'MuonHitsM4R2',  'MuonHitsM4R3',  'MuonHitsM4R4',  'MuonTracks')
# muons are added since defining the cut function addresses the issue of 'shifted' histogram for some muon sub-counters, yielding much better results.
...
# add the counters that were affected by the LED calibrations (see lines 117-146)
be_prb = list('ECalET','ECalEtot','ECalETOuterTop', 'ECalETOuterBottom', 'ECalETMiddleTop', 'ECalETMiddleBottom', 'ECalETInnerTop', 'ECalETInnerBottom','RawECalEOuterTop', 'RawECalEOuterBottom', 'RawECalEMiddleTop', 'RawECalEMiddleBottom', 'RawECalEInnerTop', 'RawECalEInnerBottom')

# in June mu scan, the be and eb, and some ee are affected. in May, it was not observed.
# after checking this, the be and/or eb bunch crossings w/ bxID <300 were excluded for those specific counters (see mu_calc() function).
# in principle, it might be better to exclude specific bxIDs instead of excluding all the bxs below a certain bxID.
# that was also done and checked.
# but considering the already multi-faceted complex measures that had to be taken, for simplicity, bxID <300 were excluded.

# it is strongly advised to check if be and/or eb were included in LED calibration bunch crossings.
# however, since the checking values or plots could vary depending on the run, an example that was appropriate for the June mu scan is presented.
# as such, this part is commented out and should only be considered as a reference.
# one should check this step in detail depending on the run.

# be_prb_dt <- data.table(file = list.files('.',pattern='^run.*gz$')
#                  )[, mu :=  as.numeric(sub('run_[0-9]+_mu_', '', sub('.gz$', '', file)))
#                    ][, {
#                         # select only the counters that could be affected
#                         . <- fread(file, col.names =  c('minute','name','bx','x','y'))[name %in% be_prb]
#                         .[,xn:=floor(x/1)*1,by=.(minute,name,bx)]
#                         .[,.(yn=sum(y)),by=.(minute,name,bx,xn)]
#                    }, by = .(file, mu)]
# be_prb_dt[,bx.type := ifelse(bx %in% bbs, 'bb', ifelse(bx %in% bes, 'be', ifelse(bx %in% ebs, 'eb', 'ee'))),by=.(file,minute,name)]
# # take into account the initial binnings from the ROOT file
# be_prb_dt[grepl('^ECal',name), xn:=xn*20][grepl('^Raw', name), xn:=xn*100]

# # plot the number of events vs. bunch crossing (bx) to check if the selected list of counters' be (or eb) bunches are affected by LED
# q(be_prb_dt[bx.type=='be'&xn>12000&name%in%Raws,.(yn=sum(yn)),by=.(bx,name)],bx,yn)+facet_wrap(~name,scales='free_y')+geom_vline(xintercept=300,linetype='dashed',color='green')+ annotate('label', x = 1000, y=100000, label=paste0('bx=300'),size = 6)
# ggsave(paste0(date,'_be_LED_Raws.jpg'))

# # plot the number of events vs. bunch crossing (bx) to check if the selected list of counters' be (or eb) bunches are affected by LED
# q(be_prb_dt[bx.type=='be'&xn>17000&name%in%ECals,.(yn=sum(yn)),by=.(bx,name)],bx,yn)+facet_wrap(~name,scales='free_y')+geom_vline(xintercept=300,linetype='dashed',color='green')+ annotate('label', x = 1000, y=100000, label=paste0('bx=300'),size = 6)
# ggsave(paste0(date,'_be_LED_ECals.jpg'))
...
cut <- function(bin0,rawbin0,per,jmp,names) {
    ...
    # additionally, if xcut == jmp, set it to zero since it most likely means the zero for that counter is really the empty events
    n1 <- n1[,.(cut=ifelse(name %in% ETots, xcut*bin0, ifelse(name %in% ECals, xcut*bin0, ifelse(name %in% Raws, xcut*rawbin0, ifelse(xcut==jmp, 0, xcut))))),by=name]
...
ratio <- function(bin0, rawbin0) {
    # create an empty data table with two rows w/ coulmn names 'name' and 'ratio'
    rt <- data.table()
    rt[,name := character(0)]
    rt[,ratio:= numeric(0)]
    ...
    for (i in 1:length(counters)){
        ...
        right <- ifelse(grepl('^ECal',counters[i]),sum(in_dt1[name==counters[i]&bx.type=='ee'&x*bin0>get_cut[name==counters[i]]$cut]$y),
            ifelse(counters[i]%in%Raws,sum(in_dt1[name==counters[i]&bx.type=='ee'&x*rawbin0>get_cut[name==counters[i]]$cut]$y),
            ifelse(counters[i]%in%names,sum(in_dt1[name==counters[i]&bx.type=='ee'&x>get_cut[name==counters[i]]$cut]$y),
...
# since the 'special' counters were not considered at the end of my analysis, it wasn't repeated.
...
mu_calc <- function(dir,bin0,rawbin0){
    ...
    dt <- data.table(file = list.files('.',pattern='^run.*gz$')
                    ...
                    print(file)
                       ...
                       # get the indices of the rows of the be and/or eb bunch crossings that were affected (see lines 117-146)
                       # for June, it was up to bxID < 300
                       prb_bx <- which(.$name%in%be_prb & .$bx < 300)
                       # remove those bunch crossings from the following analysis
                       . <- .[-prb_bx,]
...
write.fst(jmu, "C:/Users/saimum/R/plume/mu_scan_june/re/jmu_121323.fst")

# read the locally stored data
jmu <- read.fst('C:/Users/saimum/R/plume/mu_scan_june/re/jmu_121323.fst',as.data.table=TRUE)
...
# for the counters that were affected by LED calibration, only be signals are subtracted
jmu1[, lz := ifelse(name %in% be_prb, lz_mu.bb - lz_mu.be, lz_mu.bb - lz_mu.be - lz_mu.eb + lz_mu.ee)]

# if no issues w/ LED calibration, the following simpler line of code may be executed
jmu1[, lz := lz_mu.bb - lz_mu.be, lz_mu.bb - lz_mu.be - lz_mu.eb + lz_mu.ee]
...
## optional
# if the mu values from the average method were also obtained, for easier calculation, take the avg ones out (or log zero ones if mu from the avg method is desired)
...
# not really needed; just that i prefer to save files by date and the version on each date for ease of access.
date = 121323
ver = 1


# to make the plots look nicer
theme_set(theme_get() + theme_bw() + theme(title = txt.16, axis.title = txt.16, 
                                           legend.title = txt.16, legend.text = txt.16,
                                           axis.text = txt.16, strip.text = element_text(size = 16)))
...
print(q(pt4, VeloVertices, x, size = I(0.4)) + facet_wrap(~name, scale='free')+ geom_smooth(method=lm, formula=y~x, linewidth = 0.5) +
...
rel_res <- pt4[name !=  'mu', {
        ...
        # can also set the intersection to zero
        fit <- lm(x ~ VeloVertices)
        # get the values obtained from the linear model
        vl <- fit$fitted.values - coef(fit)[1]
        # calculate the relative residual
        .(rel_res = (x - fit$fitted.values)/vl, x = x, VeloVertices)
    }, by = name]
...
print(q(rel_res, VeloVertices, rel_res, size = I(0.6)) + facet_wrap(~name, scale='free')+ geom_smooth(method=lm, formula=y~x+0) +
    labs(y = paste0('Residual of linear fit (Counter = a*VeloVertices + b) / fit value')))
ggsave(paste0(date,'_',ver,'_rel_res_VV.pdf'),width = 297, height = 210, units = "mm")


## re-do the process for other desired counters
# example presented for another counter - 'VeloTracks'

pt5 <- melt(jmu3, id.vars = c('file','mu','minute','lumi','n.bb','VeloTracks'),variable.name='name',value.name='x')
print(q(pt5, VeloTracks, x, size = I(0.4)) + facet_wrap(~name, scale='free')+ geom_smooth(method=lm, formula=y~x, linewidth = 0.5) +
...
rel_res_vt <- pt5[name !=  'mu', {
        fit <- lm(x ~ VeloTracks)
        vl <- fit$fitted.values - coef(fit)[1]
        .(rel_res = (x-fit$fitted.values)/vl, x = x, VeloTracks)
    }, by = name]

print(q(rel_res_vt, VeloTracks, rel_res, size = I(0.6)) + facet_wrap(~name, scale='free')+ geom_smooth(method=lm, formula=y~x+0) +
    labs(y = paste0('Residual of linear fit (Counter = a*VeloTracks + b) / fit value')))
